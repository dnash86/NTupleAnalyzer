...
def Plot(selection, emuselection,  drawSub, binning, variable, variable_emu, xlabel,zscale,ttscaler,tag):
    ...
    h_Data=MakeHisto('h_Data','t#bar{t} data, e#mu sel',emu_SingleMuData,variable_emu,binning,emuselection+Trigger_emu+Filters,TTStackStyle,Label)
    ...
    File.write('{$t\\bar{t}$}  & {'+str(round(h_TTBar_emusel.Integral(),2))+'} \\\\\n')
    File.write('{Z}  & {'+str(round(h_emu_ZJets.Integral(),2))+'} \\\\\n')
    File.write('{Single Top}  & {'+str(round(h_emu_SingleTop.Integral(),2))+'} \\\\\n')
    File.write('{DiBoson}  & {'+str(round(h_emu_DiBoson.Integral(),2))+'} \\\\\n')
    File.write('{W}  & {'+str(round(h_emu_WJets.Integral(),2))+'} \\\\\n')
    File.write('\\hline\n')
    File.write('\\hline\n')
