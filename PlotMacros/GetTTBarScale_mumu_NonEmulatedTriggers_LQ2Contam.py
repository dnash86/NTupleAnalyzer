...
Files_LQ2='/store/user/dnash/LQAnalyzerOutput2/NTupleAnalyzer_V00_02_06_David_2012_Foremu_LQ2Signal_ForEMu_2014_08_20_15_15_35/SummaryFiles'
...
for f in os.popen('cmsLs '+Files_LQ2+'| grep ".root" | gawk \'{print $NF}\' | gawk -F "/" \'{print $NF}\'').readlines():
    print f.replace('-','_').replace(".root\n","")
    print f.replace('-','_').replace(".root\n","")+" = TFile.Open(\"root://eoscms//eos/cms/"+Files_LQ2+"/"+f.replace("\n","")+"\")"+".Get(\""+TreeName+"\")"
    exec(f.replace('-','_').replace(".root\n","")+" = TFile.Open(\"root://eoscms//eos/cms/"+Files_LQ2+"/"+f.replace("\n","")+"\")"+".Get(\""+TreeName+"\")")
...
def Plot(selection, emuselection,  drawSub, binning, variable, variable_emu, xlabel,zscale,ttscaler,tag):
    ...
    SignalStyle1=[0,22,0.7,3,1]
    SignalStyle2=[0,22,0.7,3,2]

    h_Signal300=MakeHisto('h_Signal300','LQuujj300',LQuujj300,variable_emu,binning,emuselection+LumiAndPU+SingleMuNonEmulatedTrigger+Filters,SignalStyle1,Label)
    h_Signal600=MakeHisto('h_Signal600','LQuujj600',LQuujj600,variable_emu,binning,emuselection+LumiAndPU+SingleMuNonEmulatedTrigger+Filters,SignalStyle2,Label)
    ...
    print "Signal300 : " +str(h_Signal300.Integral())
    print "Signal600 : " +str(h_Signal600.Integral())
    ...
    h_Signal300.SetLineStyle(3)
    h_Signal300.Draw("HISTSAME")

    h_Signal600.SetLineStyle(3)
    h_Signal600.Draw("HISTSAME")
    ...
    leg.AddEntry(h_Signal300,"LQ2 Signal Mass 300 GeV")
    leg.AddEntry(h_Signal600,"LQ2 Signal Mass 600 GeV")
...
def MakePlots():
    ...
    DrawHisto(Selection, Selection_emu, drawSub, mbinning, "M_singleLQ_mupfjet_Masshigh", "M_singleLQ_emusel_Masshigh", "M_{l jet} " +xtag, znorm, ttscaler,filetag)
