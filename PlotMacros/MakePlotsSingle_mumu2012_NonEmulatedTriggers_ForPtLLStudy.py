...
def DrawHisto(JustIntegrate,lq_choice, selection, emuselection, qcdselection, use_emu, drawSub, binning, variable, variable_emu, variable_qcd, xlabel,zscale,ttscaler,tag):
    ...
    if drawSub:
        ...
        xminl = 0
        xmaxl = 0
	print "Uncertainties, with bin edges:"
        for ibin in range(nbinsx):
            ...
            if (ndat!=0 and nbg != 0):
                ...
                #print "This bin = "+str((ndat - nbg)/nbg) +"+/-" +str((err_total)/nbg)
...
def plot():
    ...
    DrawHisto(JustIntegrate,lq_choice, Selection, Selection_emu, Selection_qcd, false, drawSub, ptllbinning, "(Pt_muon1+Pt_muon2)", "(Pt_muon1+Pt_muon2)","", "p_{T}(ll) (GeV)" +xtag, znorm, ttscaler,filetag)
    return
