...
import array
import math
from optparse import OptionParser  #Available in 53X
#import argparse   #Cannot be used in 53X
tRand = TRandom3()
...
_kinematicvariables += ['St_uuj','St_eej']
_kinematicvariables += ['M_uu','M_ee','MT_uv']
...
_kinematicvariables += ['M_ej','M_e1j','M_uj','M_u1j']
...
def FullKinematicCalculation(T,variation):
	...
	#### Single LQ ST and M computation
	_stuuj = ST([muons[0],muons[1],jets[0]])
	_steej = ST([electrons[0],electrons[1],jets[0]])
	...
	_Mu1j = (muons[0]+jets[0]).M()
	_Muj=GetLLJMass(muons[0],muons[1],jets[0])
	_Me1j = (electrons[0]+jets[0]).M()
	_Mej=GetLLJMass(electrons[0],electrons[1],jets[0])

	#### End single LQ ST and M computation
	...
	toreturn += [_stuuj,_steej]
	toreturn += [_Muu,_Mee,_MTuv]
	...
	toreturn += [_Mej,_Me1j,_Muj,_Mu1j]
