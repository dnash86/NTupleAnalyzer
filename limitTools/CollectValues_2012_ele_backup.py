...
def SaveValuesForCards():
    ...
    TotalUncertainty_MC = MuScaleImpact_MC**2+JetScaleImpact_MC**2+MuSmearImpact_MC**2+JetSmearImpact_MC**2+LumiImpact_MC**2+PUImpact_MC**2+ZShapeImpact_MC**2+ZNNPDFImpact**2

    TotalUncertainty_Sig = MuScaleImpact_sig**2+JetScaleImpact_sig**2+MuSmearImpact_sig**2+JetSmearImpact_sig**2+PUImpact_sig**2+SigNNPDFImpact**2
...
def NPlaces(x):
    ...
    if Dec > 1:
        ...
        if StringX.split('.')[1]!="0":
            ...
            if len(StringX.split('.')[1]) >1:
                
                if int(str(StringX.split('.')[1])[-2]) =="0":
                    return -1*(Ints-1)
                else:
                    return -1*(Ints-2)
            else:
...
def WriteCards():
    ...
    ComputedSystematics2_MC = MuScaleImpact_MC**2+JetScaleImpact_MC**2+MuSmearImpact_MC**2+JetSmearImpact_MC**2+LumiImpact_MC**2+PUImpact_MC**2+ZShapeImpact_MC**2+ZNNPDFImpact**2
    ...
    siground = NPlaces(round_sig(localsigerror,2))
    ...
    MCround = NPlaces(round_sig(localTotalStatError,2))
    ...
    if MC > localTotalStatError:
        MCString+= "\pm "+str(round_sig(localTotalStatError,2))
    else:
        MCString+= "^{+"+str(round_sig(localTotalStatError,2))+"}_{-"+str(round(MC,MCround))+"}"

    print "-------Systematic is " + str(localTotalSystError_MC)
    print "-------MC is " + str(MC)
    print "-------MCround is " + str(MCround)
    #This is a hack:
    ...
    if (z > localzerror):
        ZString = str(round(z,zround))+"\pm "+str(round_sig(localzerror,2))
    else:
        #ZString = str(round(z,zround))+"^{+"+str(round_sig(localzerror,2))+"}_{-"+str(round_sig(z,2))+"}(Syst.)"
        ZString = str(round(z,zround))+"^{+"+str(round_sig(localzerror,2))+"}_{-"+str(round(z,zround))+"}"

    if (tt > localtterror):
        TTString = str(round(tt,ttround))+"\pm "+str(round_sig(localtterror,2))
    else:
        #TTString = str(round(tt,ttround))+"^{+"+str(round_sig(localtterror,2))+"}_{-"+str(round_sig(tt,2))+"}"
        TTString = str(round(tt,ttround))+"^{+"+str(round_sig(localtterror,2))+"}_{-"+str(round(tt,ttround))+"}"

    if (BGleftover > localLeftoverStatError):
        LeftoverString = str(round(BGleftover,BGleftoverround))+"\pm "+str(round_sig(localLeftoverStatError,2))
    else:
        #LeftoverString = str(round(BGleftover,BGleftoverround))+"^{+"+str(round_sig(localLeftoverStatError,2))+"}_{-"+str(round_sig(BGleftover,2))+"}"
        LeftoverString = str(round(BGleftover,BGleftoverround))+"^{+"+str(round_sig(localLeftoverStatError,2))+"}_{-"+str(round(BGleftover,BGleftoverround))+"}"
